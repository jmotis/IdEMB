...
def find_librarians():
    ...
    results = open('resultsDebt.csv', 'w+')
    ...
    while (i<7):
        ...
        for line in entry:
            ...
            statement_type = 'debt'
            start_index = 0
            for mention in re.finditer('indebted to', line, flags=re.IGNORECASE):
                start_index = mention.start()
                if (start_index > 0):
                    snippet = line[start_index-65:start_index+65]
                    results.write(ID + ',' + statement_type + ',' + '"' + snippet + '"' + '\n')
            start_index = 0
            for mention in re.finditer('following libraries', line, flags=re.IGNORECASE):
                start_index = mention.start()
                if (start_index > 0):
                    snippet = line[start_index-25:start_index+105]
                    results.write(ID + ',' + statement_type + ',' + '"' + snippet + '"' + '\n')

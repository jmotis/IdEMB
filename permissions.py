...
def find_librarians():
    ...
    while (i<7):
        ...
        for line in entry:
            ...
            start_index = 0
            for mention in re.finditer('courtesy of', line, flags=re.IGNORECASE):
                start_index = mention.start()
                if (start_index > 0):
                    snippet = line[start_index-65:start_index+65]
                    results.write(ID + ',' + statement_type + ',' + '"' + snippet + '"' + '\n')
            start_index = 0
            for mention in re.finditer('permission', line, flags=re.IGNORECASE):
                start_index = mention.start()
                if (start_index > 0):
                    snippet = line[start_index-65:start_index+65]
                    results.write(ID + ',' + statement_type + ',' + '"' + snippet + '"' + '\n')

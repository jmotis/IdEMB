...
def find_librarians():
    ...
    while (i<7):
        ...
        for line in entry:
            ...
            statement_type = 'STC'
            start_index = 0
            for mention in re.finditer('STC', line, flags=re.IGNORECASE):
                start_index = mention.start()
                if (start_index > 0):
                    snippet = line[start_index-75:start_index+30]
                    results.write(ID + ',' + statement_type + ',' + '"' + snippet + '"' + '\n')
              # find permission statements
            statement_type = 'Wing'
            start_index = 0
            for mention in re.finditer(' Wing', line):
                start_index = mention.start()
                if (start_index > 0):
                    snippet = line[start_index-75:start_index+12]
                    results.write(ID + ',' + statement_type + ',' + '"' + snippet + '"' + '\n')
